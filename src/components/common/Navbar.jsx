...
const Navbar = () => {
  ...
  // 1. Public Links (Unauthenticated)
  const publicLinks = [
  ...
  const technicianLinks = [
    ...
    { name: 'Availability', path: '/technician/profile#availability' },
  ...
  return (
    <nav className="sticky top-0 z-40 glass-nav transition-all duration-200 border-b border-slate-200/90 shadow-xs">
        ...
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-4">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="text-slate-900 font-extrabold text-xl leading-tight tracking-tight">
                Service<span className="text-blue-600">Hub</span>
              </span>
            </Link>
          </div>

          {/* Desktop Navigation Links (Overflow protected) */}
          ...
          {/* Medium screen navigation fallback (when between md and lg) */}
          <div className="hidden md:flex lg:hidden items-center space-x-1 overflow-x-auto no-scrollbar max-w-[45vw] py-1">
            {currentNavLinks.slice(0, 4).map((link) => (
              <Link
                key={link.path}
                to={link.path}
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex items-center gap-2 pl-2 pr-2.5 py-1 rounded-xl bg-white border border-slate-200 shadow-2xs text-xs">
                  ...
                  <span className="font-semibold text-slate-800 max-w-[100px] xl:max-w-[130px] truncate">
                ...
                <button
                  ...
                  className="text-xs font-semibold text-slate-600 hover:text-red-600 hover:bg-red-50 px-2.5 py-1.5 rounded-xl transition-colors"
                >
                  Sign Out
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  ...
                  className="text-xs font-bold bg-slate-900 hover:bg-slate-800 text-white px-3.5 py-2 rounded-xl transition-all shadow-xs hover-lift"
                >
                  Join as Technician
                </Link>
              </div>
      ...
      {/* Mobile Menu Dropdown (Fully responsive, no overflow) */}
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-200 bg-white/98 backdrop-blur px-4 pt-3 pb-5 space-y-2 shadow-lg max-h-[85vh] overflow-y-auto">
          <div className="space-y-1">
            ...
            {/* If Customer, also show Profile in mobile menu */}
            ...
            {isAuthenticated ? (
              <>
                <div className="p-2.5 text-xs bg-slate-50 rounded-xl flex items-center justify-between border border-slate-200">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border flex-shrink-0 ${roleBadgeColor}`}>
                    {role === 'PROVIDER' ? 'TECHNICIAN' : role}
                  </span>
                </div>
                <button
